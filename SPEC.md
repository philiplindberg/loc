...
**`--by-file`.** With `--by-file`, each language row is followed by a rule, one row per file counted under it, and a rule, so that every language is a framed section; the last section's closing rule is the one above the total row. A file row's first column is the file's path as `loc` reached it, the `PATH` given joined with the path beneath it, with a leading `./` dropped; a path longer than 40 characters loses whole leading directories, replaced by `…/` (U+2026, `/`), until it is 40 or fewer or only the file name is left, so the file name is never cut and only a name over 40 can widen the column; `Files` is empty; `Lines`, `Blank`, `Comment`, `Code`, and `%` are the file's own, `%` its share of total code. Files sort by code descending, ties by path ascending compared byte-wise. Column widths include these rows as shown, so the first column is as wide as the longest shown path. A path that is not valid UTF-8 is shown with U+FFFD in place of each invalid sequence. Styled, a file row carries the two-space prefix like the header and is otherwise plain. In JSON, each language object gains a last key `by_file`, an array of `{"path","lines","blank","comment","code","bytes"}` objects in the same order; the key is absent without the flag.
...
**`--exclude-lang`.** Each `--exclude-lang NAME` names a language as `--languages` prints it, compared without regard to ASCII case; any other name is a usage error. A file recognized as an excluded language is neither counted nor reported: it is absent from the table, the skipped section, and every file total, like a path an `--exclude` pattern removes. Recognition is unchanged, so a `.c` file is C whatever it holds, and once recognized the file is not read, so one holding a `NUL` byte or one that cannot be read produces no label and no warning. Embedded regions stay with their file's language: excluding CSS leaves the styles inside an HTML, Vue, Svelte, or PHP file counted under that file. The names apply with `--no-ignore` too, and `--languages` lists every language regardless.
...
**Wildcards.** Matching is byte-wise and case-sensitive. `*` matches zero or more bytes other than `/`; `?` matches one byte other than `/`; `[…]` matches one byte other than `/` from the set, where `a-z` is a range, a leading `!` or `^` negates the set, and `\` escapes; a `[` with no closing `]` makes the line match nothing, as it does in git.
...
| C# | `.cs` | `//` | `/* */` | no | `"…"` single, escapes; `'…'` char; `@"…"`, `$@"…"`, `@$"…"` multi, no escapes; `"""…"""` multi, no escapes | `#7355dd` |
...
C#'s `$"…"` interpolation is not tracked, so a quote inside its `{…}` opens a phantom that ends at the newline, and `""` inside a verbatim string closes and reopens it. Swift's `\(…)` interpolation is not tracked, so a quote inside one opens a phantom that ends at the newline, and its `#"…"#` raw strings are read as ordinary strings. Scala's `${` is read as interpolation in every double-quoted string, prefixed or not. Groovy's slashy strings (`/…/`) are code. A `.m` file is Objective-C, never MATLAB, and `.mm` counts as Objective-C rather than Objective-C++. An XML `CDATA` section is not parsed, so `<!--` inside one opens a comment. A `<style lang="scss">` block in Vue or Svelte is scanned by CSS's rules, so its `//` comments are code. In PHP, a `?>` inside a `//` or `#` comment does not end the region as it does for PHP itself, `$name` interpolation without braces is not tracked, the short `<?` tag is not recognized, and a `#!` first line lies outside any region and is code.
